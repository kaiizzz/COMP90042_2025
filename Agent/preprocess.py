...
def return_original_df():
    return (
        pd.DataFrame(train_claims),
        pd.DataFrame(dev_claims),
        pd.DataFrame(dev_baseline_claims),
        pd.DataFrame(test_claims),
        pd.DataFrame(list(evidence.items()), columns=["key", "value"]),
    )
